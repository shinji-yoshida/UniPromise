...
namespace UniPromise {
	public interface Promise<T> : IDisposable {
		State State { get; }
		...
		Promise<U> Then<U>(Func<T, Promise<U>> done);

		Promise<U> Then<U>(Func<T, Promise<U>> done, Func<Exception, Promise<U>> fail);

		Promise<U> Then<U>(
			Func<T, Promise<U>> done, Func<Exception, Promise<U>> fail, Func<Promise<U>> disposed);

		Promise<U> Select<U> (Func<T, U> selector);
